...
Backend.prototype.get = function(tableidx, data, cb) {
  this.select(tableidx, data, function(err, rows) {
    ...
    // accept row with largest year & timestamp
    var row = null;
    _.each(rows, function(r) {
      if (!row || (parseInt(r.year, 10) > parseInt(row.year, 10)) || (parseInt(r.year, 10) === parseInt(row.year, 10) && r.timestamp > row.timestamp)) {
        row = r;
      }
    });
...
Backend.prototype.getEntry = function(data, cb) {
  ...
  if (data.exactyear) {
    ...
    year = {
      '<=': data.year || config.get('display_year')
    };
  }
  var queryInfo = {
    place: data.place,
...
Backend.prototype.updateEntry = function(idInfo, newdata, cb) {
  var self = this;
  // SET exactyear so that 'updates' really get
  // entered in the year they are submitted.
  idInfo.exactyear = true;
  // check if an existing entry
  this.getEntry(idInfo, function(err, entry) {
    // TODO: handle errors ...
...
Backend.prototype.processSubmission = function(user, acceptSubmission, submissionid, reviewData, callback) {
  ...
  function processSubmission(submission) {
    ...
    submission.reviewcomments = reviewData.reviewcomments;
    if (acceptSubmission) {
...
Backend.prototype.acceptSubmission = function(submission, newdata, cb) {
  var self = this;
  this.updateEntry(submission, newdata, onEntryUpdate);
...
// get all the entries with additional info
